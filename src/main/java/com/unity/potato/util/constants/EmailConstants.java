package com.unity.potato.util.constants;

public class EmailConstants {
    //public static final String TITLE_CERTIFICATION = ;
    public static final String TITLE_EMAIL_CODE_CHECK = "[감자조아] 이메일 인증 코드 발급";
    public static final String TITLE_EMAIL_GET_PWD = "[감자조아] 임시 비밀번호 발급";
    public static final String HOST_NAME = "감자조아";
    public static final String PREFIX_CERTIFICATION = "email certification:";
    public static final String PREFIX_TRY_CERTIFICATION = "email try count:";
    public static final String PREFIX_TRY_VERIFICATION = "email code try count:";
    public static final String PREFIX_FIND_PWD_TRY = "email find password count:";
    public static final String EMAIL_VERIFICATION_ID = "email cert success:";
}
